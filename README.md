...
1. **Nettoyage & exploration des données** (`final_salesdaily_cleaned.csv`)
...
```
.
├── projet_prediction_stock.ipynb   # Notebook complet (EDA, feature engineering, entraînement, API)
├── README.md                       # Documentation (ce fichier)
├── requirements.txt                # Dépendances Python
├── .gitignore                      # Fichiers à exclure du dépôt
└── data/
    └── final_salesdaily_cleaned.csv  # Jeu de données (à ajouter, voir ci-dessous)
```
...
2. Importez `projet_prediction_stock.ipynb` (`Fichier > Importer un notebook`)
3. Importez le fichier de données `final_salesdaily_cleaned.csv` dans l'environnement Colab (panneau de gauche > icône dossier > glisser-déposer)
...
# Windows (PowerShell)
$env:NGROK_AUTH_TOKEN="votre_token_ici"
```
...
# {"medicament": "N05C", "date": "2027-08-15", "valeur_predite": 0.06}

curl "http://localhost:8000/recommandation?medicament=N05C&date=2027-08-15"
# {"medicament": "N05C", "date": "2027-08-15", "stock_actuel": 177.0, "valeur_predite": 0.06, "statut": "STOCK SUFFISANT"}

curl "http://localhost:8000/predict-most-demanded?date=2027-08-15"
# {"date": "2027-08-15", "medicament_plus_demande": "N02BE", "valeur_predite": 10.17, ...}
```
